<?php
// Heading
$_['heading_title']      = 'PayPal Payments Pro Payflow Edition';

// Text
$_['text_payment']       = 'Zahlung';
$_['text_success']       = 'Erfolgreich: PayPal Payments Pro Payflow Edition geändert!';
$_['text_pp_pro_pf']     = '<a onclick="window.open(\'https://www.paypal.com/uk/mrb/pal=W9TBB5DTD6QJW\');"><img src="view/image/payment/paypal.png" alt="PayPal Website Payment Pro (UK)" title="PayPal Website Payment Pro (UK)" style="border: 1px solid #EEEEEE;" /></a>';
$_['text_authorization'] = 'Genehmigung';
$_['text_sale']          = 'Verkauf';

// Entry
$_['entry_vendor']       = 'Lieferant:<br /><span class="help">Die Lieferantennummer zum Login auf PayPal Payments Pro (UK).</span>';
$_['entry_user']         = 'Benutzer:<br /><span class="help">Wurden mehrere Benutzer auf PPPro angelegt, hier die ID desjenigen angeben welcher für Überweisungen berechtigt ist. Ist nur ein Berechtigter eingetragen, hier dieselbe ID wie beim Anbieter angeben.</span>';
$_['entry_password']     = 'Passwort:<br /><span class="help">Das 6 bis 32 Zeichen lange Passwort, dass während der Registrierung definiert wurde.</span>';
$_['entry_partner']      = 'Partner:<br /><span class="help">Die ID welche vom PayPal Verkäufer erhalten wurde (für die Payflow SDK). Wurde direkt bei PayPal gekauft, dann hier die PayPal UK ID angeben.</span>';
$_['entry_test']         = 'Testmodus:<br /><span class="help">Verwende dieses Modul im Test- (JA) oder Livemodus (Nein)?</span>';
$_['entry_transaction']  = 'Transaktionsmethode:';
$_['entry_total']        = 'Summe:<br /><span class="help">Der Warenkorb muss diese Summe beinhalten, damit dieses Zahlungsverfahren verfügbar ist.</span>';
$_['entry_order_status'] = 'Auftragsstatus:';
$_['entry_geo_zone']     = 'Geo Zone:';
$_['entry_status']       = 'Status:';
$_['entry_sort_order']   = 'Reihenfolge:';

// Error
$_['error_permission']   = 'Warnung: Sie haben keine Berechtigung, um PayPal Payment Pro (UK) zu ändern!';
$_['error_vendor']       = 'Lieferant erforderlich!';
$_['error_user']         = 'Benutzer erforderlich!';
$_['error_password']     = 'Passwort erforderlich!';
$_['error_partner']      = 'Partner erforderlich!';
?>